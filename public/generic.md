...
**NEVER commit changes without explicit user permission.** You may stage changes with `git add` and show the user what would be committed with `git status` or `git diff --staged`, but the actual `git commit` command should only be executed when the user explicitly asks for it. This is non-negotiable.


**DO NOT READ public directory** This directory takes up 140% of your context but it is just images
